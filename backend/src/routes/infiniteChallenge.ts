 ...
 * Formula: stage * 10.5 + stage * 2.1 (50% increase from stage * 7 + stage * 1.4)
 * Stage 1: 12P, Stage 10: 126P, Stage 50: 630P, Stage 100: 1260P
 */
  ...
  try {
    ...
    if (isVictory) {
      ...
      res.json({
        ...
        message: `스테이지 ${currentStage} 클리어! ${reward.toLocaleString()}P 획득!`,
      });
    } else {
      // Defeat - end run
      await connection.query(
